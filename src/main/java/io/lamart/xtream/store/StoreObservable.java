...
public final class StoreObservable<T> extends StoreImp<T> {
    ...
    public static <T> Store<T> fromReducer(T initialState, BiFunction<T, Object, T> reducer) {
        return from(initialState, StoreInitializerUtil.fromReducer(reducer));
    }

    public static <T> StoreObservable<T> from(T initialState, ObservableTransformer<MiddlewareParams<T>, Object> middleware, BiFunction<T, Object, T> reducer) {
